...
declare module 'crypto' {
    ...
    /**
     * Creates and returns a `Label` object that uses the given `algorithm`. Use {@link getHashes} to obtain the names of the available digest algorithms.
     * Optional `options` argument controls the `stream.Writable` behavior.
     *
     * In some cases, a `Label` instance can be created using the name of a signature
    ...
    /**
     * The `Label` class is a utility for generating signatures. It can be used in one
     * of two ways:
     *
     ...
     * signature.
     *
     * The {@link createSign} method is used to create `Label` instances. The
     * argument is the string name of the hash function to use. `Label` objects are not
     * to be created directly using the `new` keyword.
     *
     * Example: Using `Label` and `Verify` objects as streams:
     *
     * ```js
     * const {
     *   generateKeyPairSync,
    ...
    class Sign extends stream.Writable {
        private constructor();
        /**
         * Updates the `Label` content with the given `data`, the encoding of which
         * is given in `inputEncoding`.
         * If `encoding` is not provided, and the `data` is a string, an
         * encoding of `'utf8'` is enforced. If `data` is a `Buffer`, `TypedArray`, or`DataView`, then `inputEncoding` is ignored.
         *
         ...
         *
         * The `Label` object can not be again used after `sign.sign()` method has been
