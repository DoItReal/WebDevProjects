   ...
   --------- Converting Options into list (own structure) -------- */
        ...
        $(document).on("click", '.labelsContent .filter_list input[type="checkbox"]', function () {
            ...
            if ($(this).is(":checked")) {
                ...
                let span = $('<span/>', {
                    'addClass': 'option',
                    'html': png.images[Number(inputVal) - 1]
                });
                $(span).attr('data-title', inputText);
                $(".labelsContent .select").append(span);
                allergens.selectedAllergens.push(inputVal);
            }
            else {
                if ($(".labelsContent .select span").length >= 1) {
                    findVal.remove();
                    allergens.selectedAllergens.splice(allergens.selectedAllergens.indexOf(inputText), 1);
                }
                if ($(".labelsContent .select span").length < 1) {
